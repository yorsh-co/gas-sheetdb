...
class GasSheetDb {
    ...
    _resolveSpreadsheet(options) {
        const { spreadsheet, spreadsheetUrl, spreadsheetId, useActiveSpreadsheet } = options;
        // filter arguments
        const sources = {
        ...
        if (selected.length > 1) {
            throw new Error(`[GasSheetDb] Received more than one spreadsheet source option: ${selected.join(', ')}. Pass exactly one of "spreadsheet", "spreadsheetUrl", "spreadsheetId" or "useActiveSpreadsheet".`);
        }
        ...
        if (!activeSpreadsheet) {
            throw new Error('[GasSheetDb] No active spreadsheet is accessible by this script. Pass "spreadsheet", "spreadsheetUrl" or "spreadsheetId" instead.');
        }
