...
class _GasSheetDbTable {
    ...
    _deriveRowIndexes() {
        const rowIndexes = {};
        Object.entries(this.rowNumbers).forEach(([k, v]) => (rowIndexes[k] = v - 1));
        return rowIndexes;
    }
    /**
    ...
    find(options = {}) {
        ...
        return this._withLock(() => {
            ...
            const isTrashedColIndex = this._getColumnIndex(_GAS_SHEETDB_SYSTEM_FIELDS.IS_DELETED);
            const entries = [];
            for (let dataIndex = 0; dataIndex < data.length; dataIndex++) {
                const row = data[dataIndex];
                // handle `onlyTrashed = true`
                if (onlyTrashed && !row?.[isTrashedColIndex])
                    continue;
                // handle `withTrashed = false`
                if (!onlyTrashed && !withTrashed && row?.[isTrashedColIndex])
                    continue;
    ...
    _setTableBodyData(data = []) {
        ...
        const tableBodyRange = this.sheet.getRange(this.rowNumbers.firstData, 1, data.length, data[0].length);
    ...
    _mapTableBodyRowIndexesById(tableBody = null) {
        ...
        if (idColIndex === -1) {
            throw new Error(`Cannot map rows by "${_GAS_SHEETDB_SYSTEM_FIELDS.ID}": column does not exist on sheet "${this.sheetName}". Call find() at least once before update/delete.`);
        }
    ...
    _extractKeys(entries) {
        return [
            ...new Set(entries.flatMap((entry) => Object.keys(entry).filter((key) => this._isPersistedField(key)))),
        ];
    }
    // =========================
    // TABLE ENTRY HELPERS
    ...
    _ensureRequiredMetadata() {
        ...
        if (!data.length)
            return;
        const idColIndex = this._getColumnIndex(_GAS_SHEETDB_SYSTEM_FIELDS.ID);
        const createdAtColIndex = this._getColumnIndex(_GAS_SHEETDB_SYSTEM_FIELDS.CREATED_AT);
        const updatedAtColIndex = this._getColumnIndex(_GAS_SHEETDB_SYSTEM_FIELDS.UPDATED_AT);
        const isTrashedColIndex = this._getColumnIndex(_GAS_SHEETDB_SYSTEM_FIELDS.IS_DELETED);
        const now = new Date();
